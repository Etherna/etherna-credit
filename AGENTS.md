...
- **`test/EthernaCredit.Persistence.Tests`** (xUnit + Moq) — focuses on model-map deserialization: each `ModelMap` GUID/schema version is pinned by a stored BSON document and asserted to deserialize into the expected entity (`CreditDbContextDeserializationTest`, `SharedDbContextDeserializationTest`, helpers under `Helpers/`), and on the raw collection operations of `Extensions/`, whose filters are captured on a mocked collection and rendered through the real model maps (`OperationLogRepositoryExtensionsTest`).
- **`test/EthernaCredit.Tests`** (xUnit + Moq) — mirrors the host project: the CORS policy test (`Extensions/CorsPolicyBuilderExtensionsTest`) builds the policy from an in-memory configuration and evaluates real requests through the framework's `CorsService`, asserting on the response headers; the admin user page model test (`Areas/Admin/Pages/Users/UserModelTest`) runs `OnGetAsync` over mocked db contexts and pins the text the view shows (views keep no logic of their own: what they print is composed by the page model, where it can be tested); the API exception mapping test (`Areas/Api/ExceptionHandlerTest`) throws real driver exceptions through `ExceptionHandler.RunAsync` and asserts on the status code of the result; the logging enrichment test (`Extensions/LoggerEnrichmentConfigurationExtensionsTest`) logs real driver exceptions through a logger built with `WithCreditExceptionDetails` and asserts on the `ExceptionDetail` property of the emitted event; the API mapper test (`Areas/Api/CreditApiMapperTest`) maps the real routes over `AddCreditApi` on a test server (`Microsoft.AspNetCore.TestHost`), with the handler's dependencies mocked and the authorization policies opened, and drives them with HTTP requests, asserting on the responses.
...
- Every route delegates to the handler interface `ICreditApiHandler`, implemented by the sealed `CreditApiHandler`. `Extensions/ServiceCollectionExtensions.AddCreditApi` registers what the routes run on (the scoped handler and the minimal API validation), and `Program` calls it. Handler methods return `Task<IResult>` and wrap their body in `ExceptionHandler.RunAsync(async () => { … })`, which logs the exception and maps it to the status code (400 argument/format/invalid entity type, 401 unauthorized, 404 not found, 409 a Mongo `TransientTransactionError` the db context couldn't retry away, so the client can repeat the request, 503 Swarm api errors, 500 anything else).
...
- DTOs use the `Dto` suffix (`Areas/Api/DtoModels/`: `CreditDto`, `OperationLogDto`); request data is bound from route/query parameters, with validation attributes in the mapper: they are enforced by the validation services of `AddCreditApi` (`services.AddValidation()`, without it the attributes only decorate the OpenAPI document), which answer 400 with an `HttpValidationProblemDetails` body before the handler runs. Declare `.ProducesValidationProblem()` on a route carrying validation attributes. Deprecated endpoints stay mapped and get `.IsDeprecated(...)` route metadata instead of being removed.
...
- **Every balance change records an `OperationLog`.** The `OperationLogBase` hierarchy is a create-only audit trail: API balance updates write an `UpdateOperationLog` (author = calling service's client id), one cumulative log per day, author, reason, applied flag and user: `Persistence/Extensions/OperationLogRepositoryExtensions.TryIncrementSameDayUpdateLogAsync` adds the amount to the log of the day atomically, and the handler creates a new log only when there is none (two concurrent first updates of a day can both create theirs: the later updates merge into one of them). `OperationLogMap` doesn't store `IsApplied` when it is true, so a filter on it must match an applied log as `IsApplied != false`, never as an equality with true; the deposit/withdraw Razor flows write `DepositOperationLog`/`WithdrawOperationLog`, admin edits write `AdminUpdateOperationLog`, and first contact writes a `WelcomeCreditDepositOperationLog`.
...
- **Log events carrying a Mongo driver exception are filtered before Elasticsearch.** `Program.ConfigureLogging` enriches every event with its destructured exception through `Extensions/LoggerEnrichmentConfigurationExtensions.WithCreditExceptionDetails`, whose `Configs/Logging/MongoExceptionPropertyFilter` keeps only the scalar properties the driver declares on its exceptions (codes, names, messages, flags, error labels), outer or inner, and drops its object graphs (`Command`, `Result`, `Query`, `WriteError`, `ConnectionId`, ...): destructured by reflection they expand into hundreds of nested fields that clash with the stream mapping, and Elasticsearch 9 then answers 201 while redirecting the event to the failure store `logs-ethernacredit-<env>::failures`, invisible in the main stream and to the sink. The sink's own failures (transport errors, hard rejections) reach Serilog's self log, enabled on the console.
