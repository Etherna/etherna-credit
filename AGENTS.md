...
- **`test/EthernaCredit.Tests`** (xUnit + Moq) — mirrors the host project: the CORS policy test (`Extensions/CorsPolicyBuilderExtensionsTest`) builds the policy from an in-memory configuration and evaluates real requests through the framework's `CorsService`, asserting on the response headers.
